...
private:
  ...
  double* v; // указатель говорит о том, что это динамический массив
...
public:
 ...
 endl;
  ...
  matr& obrDiag(matr& m){
    for (int i = 0; i < dim; i++){
      if (m.b[i][i] == 0){
        cout << "ERROR: Нельзя делить на ноль! У вас нули на главной диагонали. Поэтому я возвращаю вам вашу изначальную матрицу." << endl;
        return m;
      }
    }
...
void jacobi(matr& A, vector& B){
  ...
  if (c!=10000){
  cout << "Число итераций для метода Якоби: " << c << endl;
   X.print();
  }
  else{
    cout << "Error: Превышено число итераций для метода Якоби: " << c << endl;
    cout << "Вот какие значения получились за это количество итераций: " << c << endl;
    X.print();
  }
} 
